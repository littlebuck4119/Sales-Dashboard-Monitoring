...
# บังคับ CSS ให้กลืนเป็นชิ้นเดียวกันทั้งแอป
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;800&display=swap');
    
    /* 1. พื้นหลังรวม - ไล่เฉดจากเขียว Soft ไปขาว */
    .stApp {
    ...
    /* 3. Main Content - โปร่งใสเพื่อให้เห็นพื้นหลัง Gradient */
    [data-testid="stMain"], [data-testid="stAppViewContainer"] {
        background-color: transparent !important;
    }

    /* 4. Welcome Card - ตามรูปที่พี่ต้องการ */
    .welcome-card {
    ...
    /* 6. ส่วนเสริมอื่นๆ */
    * { caret-color: transparent !important; } /* ซ่อนขีดกระพริบ */
    header, footer { visibility: hidden; }
    
    /* ปุ่มบันทึกใน Sidebar */
    button[kind="primary"] {
...
# --- 4. MAIN CONTENT (หน้า Welcome - ไม่มีปุ่ม Get Started) ---
        ...
        <div style="display: flex; justify-content: center; align-items: center; min-height: 85vh;">
            <div class="welcome-card">
                ...
                <p class="sub-title">
                    ...
                    <span style="font-size: 0.9rem; opacity: 0.7;">กรุณาเลือกแบรนด์ที่เมนูด้านซ้ายเพื่อเริ่มต้นวิเคราะห์ข้อมูล</span>
                </p>
                <div style="margin-top: 40px; padding: 15px; border-top: 1px solid rgba(0,0,0,0.03);">
                    <span style="font-size: 0.85rem; color: rgb(74, 93, 80); font-style: italic;">👈 Control panel is ready on your sidebar</span>
                </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
st.markdown(f"<h2 style='color: rgb(74, 93, 80);'>📈 {selected_brand} Dashboard</h2>", unsafe_allow_html=True)
...
if not full_df.empty:
    ...
    def style_heatmap(v):
        ...
        if v == "DISABLED": return 'background-color: #f8fafc; color: transparent;'
